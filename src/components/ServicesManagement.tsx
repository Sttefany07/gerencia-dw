import { Clock, DollarSign, Gauge, Timer } from "lucide-react";
import { ComputedRecord, FilterState, NormalizedRecord, ProjectEstimate, ServiceConsultantSummary, ServiceProjectSummary, TariffRate } from "../types";
...
export function ServicesManagement({
  ...
  const summaryColumns: Column<ServiceProjectSummary>[] = [
    ...
    { id: "saldoDisponible70", header: "Saldo costo", value: (row) => row.saldoDisponible70, render: (row) => formatMoney(row.saldoDisponible70, row.moneda), total: true, totalRender: (rows) => formatMoney(sum(rows, "saldoDisponible70"), total.moneda) }
  ];
  ...
  const consultantColumns: Column<ServiceConsultantSummary>[] = [
    ...
    { id: "costoEjecutado70", header: "Costo ejecutado", value: (row) => row.costoEjecutado70, render: (row) => formatMoney(row.costoEjecutado70, row.moneda), total: true, totalRender: (rows) => formatMoney(sum(rows, "costoEjecutado70"), total.moneda) },
    { id: "progreso", header: "Progreso", value: (row) => row.progreso, render: (row) => formatPercent(row.progreso) }
  ];
    ...
    <section className="grid gap-4 rounded-2xl bg-white p-4 shadow-soft ring-1 ring-slate-200">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm font-bold text-slate-500">Control operativo: estimado vs ejecutado, costo consumido y saldo.</p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard icon={DollarSign} label="Saldo de costo" value={formatMoney(total.saldoDisponible70, total.moneda)} />
      </div>
      <DataTable title="Resumen operativo por pais, cliente y proyecto" rows={summary} columns={summaryColumns} fileName="resumen_operativo" />
